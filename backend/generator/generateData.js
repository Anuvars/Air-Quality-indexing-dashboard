...
export default function DashboardWithTrends({ isDark }) {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const res = await axios.post("http://localhost:5000/api/air-quality", {
            lat: pos.coords.latitude,
            lon: pos.coords.longitude,
          });

          // Use UPPERCASE keys to match frontend usage
          const data = res.data;

          const metricsArray = [
            { label: "AQI", value: data.AQI, unit: "", threshold: 150 },
            { label: "PM", value: data.PM, unit: "µg/m³", threshold: 80 },
            { label: "SO₂", value: data.SO2, unit: "µg/m³", threshold: 40 },
            { label: "VOC", value: data.VOC, unit: "µg/m³", threshold: 30 },
            { label: "CO₂", value: data.CO2, unit: "ppm", threshold: 1000 },
            { label: "NO₂", value: data.NO2, unit: "ppb", threshold: 20 },
            { label: "CH₂O", value: data.CH2O, unit: "ppb", threshold: 50 },
            { label: "O₃", value: data.O3, unit: "ppb", threshold: 50 },
            { label: "TEMPERATURE", value: data.TEMPERATURE, unit: "°C", threshold: 35 },
            { label: "HUMIDITY", value: data.HUMIDITY, unit: "%", threshold: 70 },
            { label: "NOISE LEVEL", value: data.NOISE, unit: "dB", threshold: 80 },
          ];
  ...
  // 🔄 Live Trend Simulation
  useEffect(() => {
    if (metrics.length === 0) return;

    // Initialize trendData with current values
    const now = new Date();
    setTrendData([{
      time: now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      AQI: metrics.find((m) => m.label === "AQI")?.value || 0,
      CO2: metrics.find((m) => m.label === "CO₂")?.value || 0,
      Temp: metrics.find((m) => m.label === "TEMPERATURE")?.value || 0,
    }]);
  ...
  const centerMetric = metrics[0];
  const surroundingMetrics = metrics.slice(1);
  const radius = 230;
  const cardSize = 120;
  const centerX = 350;
  const centerY = 350;
  ...
  const getProgressColor = (metric) => {
    if (!metric) return "grey";
    return metric.value > metric.threshold ? "error.main" : isDark ? "#4ade80" : "#16a34a";
  };

  if (!centerMetric) return <Typography>Loading metrics...</Typography>;
  ...
  return (
    ...
    >
            ...
            <CardContent sx={{ textAlign: "center" }}>
              <Box sx={{ position: "relative", display: "inline-flex" }}>
                <CircularProgress
                  ...
                  size={90}
                  thickness={5}
                  sx={{ color: getProgressColor(centerMetric) }}
                />
                <Box
                  sx={{
                    top: 0,
                ...
                <CardContent sx={{ textAlign: "center" }}>
                  <Box sx={{ position: "relative", display: "inline-flex" }}>
                    <CircularProgress
                      ...
                      size={60}
                      thickness={4}
                      sx={{ color: getProgressColor(metric) }}
                    />
                    <Box
                      sx={{
                        top: 0,
      ...
      {/* --- Right: Alerts + Health Recommendation --- */}
          ...
          {alerts.length > 0 ? (
            alerts.map((alert, index) => (
              <Card
                key={index}
                sx={{
                  backgroundColor: isDark ? "#b91c1c" : "#fee2e2",
